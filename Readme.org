...
Use special prefix characters to force a specific matching style:

- =~foo= — flex match (scattered characters)
- ==foo= — literal match (exact substring)
- =!foo= — exclude candidates containing "foo"

Without a prefix, the default styles (literal + initialism) are used.

#+begin_src emacs-lisp
  (defun fiachetti/orderless-dispatch (word _index _total)
    "Dispatch orderless style based on prefix character."
    (cond
     ;; ~foo → flex match
     ((string-prefix-p "~" word)
      `(orderless-flex . ,(substring word 1)))
     ;; =foo → literal match
     ((string-prefix-p "=" word)
  ...
  (setq orderless-style-dispatchers '(fiachetti/orderless-dispatch))
#+end_src
...
#+begin_src emacs-lisp
  (use-package yasnippet
    ...
    :config
    (yas-reload-all)
    ;; Don't bind TAB directly to yas-expand; instead use the fallback mechanism
    ;; so that TAB indents when no snippet is available.
    (define-key yas-minor-mode-map (kbd "TAB") nil)
    (define-key yas-minor-mode-map (kbd "<tab>") nil)
    ;; Use backtab or C-tab to expand snippets explicitly (already configured below)
    )
#+end_src
...
Expanded account balances in the overall monthly report are mostly noise for me and do not convey any meaningful information.
