...
#+begin_src emacs-lisp
  (use-package orderless
    ...
    :custom
    (orderless-matching-styles '(orderless-flex orderless-initialism orderless-literal)))
#+end_src
...
Configure Tailwind CSS Language Server for HTML files and templates.

This provides IntelliSense for Tailwind CSS class names, including autocomplete,
hover previews, and linting for Tailwind-specific syntax.

The language server needs to be installed globally:
#+begin_example
  ...
  (with-eval-after-load 'lsp-mode
    (lsp-register-client
     (make-lsp-client
      ...
      :activation-fn (lsp-activate-on "html" "web")
      :priority 0
      :server-id 'tailwindcss-ls
      :major-modes '(html-mode html-ts-mode web-mode)
      :add-on? t)))
#+end_src

** Enable LSP in web-mode and HTML files
